...
function AboutAgileProjectManagement() {
  return (
    <h1 style={{ alignSelf: 'stretch', width: '100%', maxWidth: '858px', color: 'rgba(55, 56, 55, 1)', fontSize: '36px', fontWeight: '600', textAlign: 'left', lineHeight: '120.00000476837158%', margin: 0 }}>About Agile Project Management</h1>
  );
}

function AgileDescription() {
  return (
    <h3 style={{ alignSelf: 'stretch', width: '100%', maxWidth: '858px', color: 'rgba(83, 84, 83, 1)', fontSize: '18px', fontWeight: '300', textAlign: 'left', lineHeight: '139.9999976158142%', margin: 0 }}>Agile project management is a collaborative, iterative approach to planning, executing, and delivering projects. Rather than following a single, fixed plan from start to finish, agile teams work in short, focused delivery cycles — continuously gathering feedback, adjusting priorities, and refining their work as they go. The result is faster delivery of meaningful outcomes, stronger alignment with stakeholder needs, and a team culture built around transparency, adaptability, and continuous improvement. It is one of the most widely adopted approaches to project delivery in today's workplace.</h3>
  );
}

const agileReasonsList = [
  "Today's organizations operate in environments where requirements shift, markets evolve rapidly, and stakeholder expectations change over the course of a project. Traditional approaches that rely on comprehensive upfront planning often struggle to accommodate this pace of change — leading to delayed delivery, misaligned outcomes, and wasted effort.",
  "Agile project management offers a disciplined alternative. By breaking work into small, time-limited delivery cycles, teams can produce working results early and often, validate their direction with stakeholders at regular intervals, and course-correct before small issues become costly problems.",
  "Organizations across technology, financial services, healthcare, government, education, and professional services rely on agile practices to keep cross-functional teams productive, responsive, and focused on delivering the outcomes that matter most.",
  "When done well, agile project management reduces waste, improves team morale, and consistently produces results that reflect what stakeholders actually need — not just what was assumed at the outset."
];

export default function AgileManagement() { 
  return (
    <section className="agile-wrapper" style={{ marginTop: '550px' }}>
      <SideDecorativeGraphics />

      <div className="agile-content"> 
        <header className="agile-header">
          <AboutAgileProjectManagement />
          <AgileDescription />
        </header>

        <div className="agile-body">
          <ClipboardGraphic />

          <div className="agile-reasons-section">
            <h2>Why Agile Project Management Matters</h2>
            
            <div className="agile-list">
              <div className="agile-list-line"></div>
              
              {agileReasonsList.map((text, index) => (
                <div key={index} className="agile-list-item">
                  <div className="agile-list-bullet"></div>
                  <p className="agile-list-text">{text}</p>
                </div>
              ))}
            </div>
          </div>
...
const BackgroundEllipse = () => (
  <svg className="agile-bg-ellipse" width="1926" height="115" viewBox="0 0 1926 115" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const SideDecorativeGraphics = () => (
  <svg className="agile-side-cards" viewBox="0 0 451 486" fill="none" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink">
